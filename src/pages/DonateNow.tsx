...
import { Heart, Landmark, Phone, CheckCircle, Copy, ArrowRight, ArrowLeft } from "lucide-react";
...
const charityProjects = [
  { id: "orphan", name: "Orphan Sponsorship" },
  { id: "quran", name: "Quran Education Fund" },
  { id: "eid", name: "Eid Gift Program" },
...
export default function DonateNow() {
  ...
  const [formData, setFormData] = useState({
    amount: "", paymentMethod: "", project: "", fullName: "", phone: "", email: "", transactionRef: "", notes: "",
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      let receiptUrl = "";
      if (receiptFile) {
        const fileRef = ref(storage, `donation-receipts/${Date.now()}-${receiptFile.name}`);
      ...
      setStep("success");
      toast({ title: "Donation Submitted!", description: "Your donation has been recorded and is pending verification." });
    } catch (error) {
  ...
  const resetForm = () => {
    setFormData({ amount: "", paymentMethod: "", project: "", fullName: "", phone: "", email: "", transactionRef: "", notes: "" });
    setReceiptFile(null);
    setStep("amount");
  };

  const steps: Step[] = ["amount", "transfer", "confirm", "success"];
      ...
      <section className="relative overflow-hidden py-16 lg:py-24">
        ...
        <div className="container relative mx-auto px-4 text-center">
          <h1 className="mb-4 font-heading text-4xl font-bold md:text-5xl animate-slide-up">
            <span className="text-gradient">Donate Now</span>
          </h1>
          ...
          <div className="flex justify-center items-center gap-2 mb-8">
            {steps.map((s, i) => (
              <div key={s} className="flex items-center">
                ...
                {i < 3 && <div className={`w-12 h-1 mx-1 ${steps.indexOf(step) > i ? "bg-green-500" : "bg-muted"}`} />}
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="pb-16">
        <div className="container mx-auto px-4">
          {step === "amount" && (
            <Card className="mx-auto max-w-lg dark-card border-primary/30">
              <CardHeader className="text-center border-b border-primary/20 pb-6">
                ...
                <CardTitle className="font-heading text-2xl text-gradient">Make a Donation</CardTitle>
                <CardDescription>Step 1: Enter donation details</CardDescription>
              </CardHeader>
              <CardContent className="p-6 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.project} onValueChange={(v) => setFormData({ ...formData, project: v })}>
                    <SelectTrigger className="input-dark"><SelectValue placeholder="Choose a project (optional)" /></SelectTrigger>
                    <SelectContent className="bg-card border-primary/20">
                      {charityProjects.map((p) => (<SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>))}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
                ...
                <Button className="w-full btn-gold mt-4" disabled={!formData.amount || !formData.paymentMethod} onClick={() => setStep("transfer")}>
          ...
          {step === "transfer" && selectedBank && (
            <Card className="mx-auto max-w-lg dark-card border-primary/30">
              <CardHeader className="text-center border-b border-primary/20 pb-6">
                ...
                <CardDescription>Step 2: Send {formData.amount} ETB to this account</CardDescription>
              </CardHeader>
              <CardContent className="p-6 space-y-4">
          ...
          {step === "confirm" && (
            <Card className="mx-auto max-w-lg dark-card border-primary/30">
              <CardHeader className="text-center border-b border-primary/20 pb-6">
                <CardTitle className="font-heading text-2xl text-gradient">Confirm Donation</CardTitle>
                <CardDescription>Step 3: Enter your details to complete</CardDescription>
              </CardHeader>
              <CardContent className="p-6 space-y-4">
                ...
                <div className="space-y-2"><Label>Email (Optional)</Label><Input type="email" placeholder="Your email address" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="input-dark" /></div>
                <div className="space-y-2">
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <Button className="flex-1 btn-gold" onClick={handleSubmit} disabled={isSubmitting}>{isSubmitting ? "Submitting..." : "Submit Donation"}</Button>
                </div>
              </CardContent>
            </Card>
          )}
          ...
          {step === "success" && (
            <Card className="mx-auto max-w-lg dark-card border-primary/30">
              <CardContent className="p-8 text-center">
                ...
                <p className="mb-2 text-lg text-foreground">Your donation of <strong>{formData.amount} ETB</strong> has been recorded.</p>
                <p className="mb-6 text-muted-foreground">Our team will verify your payment and update your donation status.</p>
                <div className="gold-line mb-6" />
                <Button onClick={resetForm} className="btn-gold">Make Another Donation</Button>
              </CardContent>
            </Card>
          )}
        </div>
