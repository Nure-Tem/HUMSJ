...
import { Lock, Mail, BookOpen, AlertCircle, Eye, EyeOff } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Welcome to the Qirat Dashboard",
      });
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-50 to-green-50 py-12 px-4 sm:px-6 lg:px-8">
      <Card className="w-full max-w-md shadow-xl">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardTitle className="text-2xl font-bold text-emerald-700">Qirat Admin Login</CardTitle>
          <CardDescription>
            Enter your credentials to access the Qirat dashboard
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="text-center">
              <button type="button" onClick={() => setShowForgotPassword(true)} className="text-sm text-emerald-600 hover:text-emerald-800 hover:underline">
      ...
      {showForgotPassword && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <Card className="w-full max-w-md">
            <CardHeader><CardTitle className="text-emerald-700">Reset Password</CardTitle><CardDescription>Enter your email to receive a password reset link</CardDescription></CardHeader>
            <CardContent>
              {resetSent ? (
                ...
                <form onSubmit={handleForgotPassword} className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    ...
                    <Button type="submit" className="flex-1 bg-emerald-600 hover:bg-emerald-700" disabled={isLoading}>{isLoading ? "Sending..." : "Send Reset Link"}</Button>
                  </div>
                </form>
              )}
            </CardContent>
